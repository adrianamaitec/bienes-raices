...
export default function NewProperty() {
    ...
    const uploadImageToStorage = async (file: File, departmentId: number): Promise<string> => {
        const fileExt = file.name.split('.').pop();
        const fileName = `${departmentId}/${Math.random().toString(36).substring(2)}.${fileExt}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!primaryImageUrl && images.length > 0) {
                const firstImageUrl = await uploadImageToStorage(images[0].file, departmentId);
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6">
                {/* Sección de Imágenes */}
                <div className="mb-8">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">Imágenes del Departamento</h3>
                                    ...
                                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100">
                                        <div className="flex space-x-2">
                                            <button
                                                ...
                                                className={`p-1 rounded ${img.isPrimary
                                                    ? 'bg-blue-500 text-white'
                                                    : 'bg-white text-gray-700'
                                                    }`}
                                                title={img.isPrimary ? 'Imagen principal' : 'Hacer principal'}
                                            >
                                                ⭐
                                            </button>
                                            <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Ubicación */}
                    <div className="md:col-span-2">
                        <h3 className="text-lg font-medium text-gray-900 mb-4 mt-6">Ubicación</h3>
                    </div>
